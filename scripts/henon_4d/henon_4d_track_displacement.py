...
parser = argparse.ArgumentParser(
    description="Generates a random displacement and executes a given set of trackings in log scale.",
...
filename = "henon_4d_displacement_" + key + "_subid_" + sub_id + ".hdf5"
...
dest.create_dataset(
    ...
    data=np.array([
        x[:total_cond//2].reshape((side_cond, side_cond)),
        x[total_cond//2:].reshape((side_cond, side_cond))])
)
dest.create_dataset(
    "/{}/px".format(turn_list[0]),
    data=np.array([
        px[:total_cond//2].reshape((side_cond, side_cond)),
        px[total_cond//2:].reshape((side_cond, side_cond))])
)
dest.create_dataset(
    "/{}/y".format(turn_list[0]),
    data=np.array([
        y[:total_cond//2].reshape((side_cond, side_cond)),
        y[total_cond//2:].reshape((side_cond, side_cond))])
)
dest.create_dataset(
    "/{}/py".format(turn_list[0]),
    data=np.array([
        py[:total_cond//2].reshape((side_cond, side_cond)),
        py[total_cond//2:].reshape((side_cond, side_cond))])
)
